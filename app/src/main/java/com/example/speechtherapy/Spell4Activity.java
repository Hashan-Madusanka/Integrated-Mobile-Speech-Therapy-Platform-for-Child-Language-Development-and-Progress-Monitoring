...
public class Spell4Activity extends AppCompatActivity {

    ImageView mic1, mic2, mic3, correct1, correct2, correct3;
    ImageView back;
    Button next;
    private static final int SPEECH_REQUEST_CODE = 1;
    private List<String> validWords = Arrays.asList("tweet", "tweet tweet", "tweet tweet tweet"); // Valid pronunciations
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.spell4);
        ...
        mic1 = findViewById(R.id.mic10);
        mic2 = findViewById(R.id.mic11);
        mic3 = findViewById(R.id.mic12);
        correct1 = findViewById(R.id.correct10);
        correct2 = findViewById(R.id.correct11);
        correct3 = findViewById(R.id.correct12);
...
// Set correct indicators based on Firebase data
        ...
        Uri videoUri = Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.bird);
        ...
        // Set click listeners
        mic1.setOnClickListener(v -> startSpeechRecognition("mic10"));
        mic2.setOnClickListener(v -> startSpeechRecognition("mic11"));
        mic3.setOnClickListener(v -> startSpeechRecognition("mic12"));
    }
    ...
    private void startSpeechRecognition(String mic) {
        ...
        intent.putExtra(RecognizerIntent.EXTRA_PROMPT, "Lets try Tweet Tweet");
    ...
    private void updateFirebaseAndUI(boolean isCorrect) {
        if (currentMic.equals("mic10")) {
            userRef.child("mic10").setValue(isCorrect);
            correct1.setVisibility(isCorrect ? View.VISIBLE : View.INVISIBLE);
        } else if (currentMic.equals("mic11")) {
            userRef.child("mic11").setValue(isCorrect);
            correct2.setVisibility(isCorrect ? View.VISIBLE : View.INVISIBLE);
        } else if (currentMic.equals("mic12")) {
            userRef.child("mic12").setValue(isCorrect);
    ...
    private void updateCorrectIndicatorsFromFirebase() {
        userRef.child("mic10").get().addOnCompleteListener(task -> {
        ...
        userRef.child("mic11").get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult().getValue() != null) {
        ...
        userRef.child("mic12").get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult().getValue() != null) {
    ...
    public void opennextActivity() {
        Intent intent = new Intent(this,Spell5Activity.class);
        startActivity(intent);
    }
}
