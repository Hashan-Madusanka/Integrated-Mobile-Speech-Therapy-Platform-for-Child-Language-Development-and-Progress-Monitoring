...
public class Spell3Activity extends AppCompatActivity {

    ImageView mic1, mic2, mic3, correct1, correct2, correct3;
    ImageView back;
    Button next;
    private static final int SPEECH_REQUEST_CODE = 1;
    private List<String> validWords = Arrays.asList("momo", "Moo Moo", "Moo Moo Moo"); // Valid pronunciations
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.spell3);
        ...
        mic1 = findViewById(R.id.mic7);
        mic2 = findViewById(R.id.mic8);
        mic3 = findViewById(R.id.mic9);
        correct1 = findViewById(R.id.correct7);
        correct2 = findViewById(R.id.correct8);
        correct3 = findViewById(R.id.correct9);
...
// Set correct indicators based on Firebase data
        ...
        Uri videoUri = Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.cow);
        ...
        // Set click listeners
        mic1.setOnClickListener(v -> startSpeechRecognition("mic7"));
        mic2.setOnClickListener(v -> startSpeechRecognition("mic8"));
        mic3.setOnClickListener(v -> startSpeechRecognition("mic9"));
    }
    ...
    private void startSpeechRecognition(String mic) {
        ...
        intent.putExtra(RecognizerIntent.EXTRA_PROMPT, "Lets try Moo Moo");
    ...
    private void updateFirebaseAndUI(boolean isCorrect) {
        if (currentMic.equals("mic7")) {
            userRef.child("mic7").setValue(isCorrect);
            correct1.setVisibility(isCorrect ? View.VISIBLE : View.INVISIBLE);
        } else if (currentMic.equals("mic8")) {
            userRef.child("mic8").setValue(isCorrect);
            correct2.setVisibility(isCorrect ? View.VISIBLE : View.INVISIBLE);
        } else if (currentMic.equals("mic9")) {
            userRef.child("mic9").setValue(isCorrect);
    ...
    private void updateCorrectIndicatorsFromFirebase() {
        userRef.child("mic7").get().addOnCompleteListener(task -> {
        ...
        userRef.child("mic8").get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult().getValue() != null) {
        ...
        userRef.child("mic9").get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult().getValue() != null) {
    ...
    public void opennextActivity() {
        Intent intent = new Intent(this,Spell4Activity.class);
        startActivity(intent);
    }
}
