...
public class Song2_readingActivity extends AppCompatActivity {
    ...
    private List<List<String>> acceptableVariations = Arrays.asList(
            Arrays.asList("අහසේ ගෙරවිලි"),
            Arrays.asList("හඩ පැතිරෙන්නේ", "හඬ පැතිරෙන්නේ"),
            Arrays.asList("බිංදු බිංදු වැහි", "බින්දු බින්දු වැහි"),
            Arrays.asList("බිමට වැටෙන්නේ"),
            Arrays.asList("විදුලි කොටන්නේ"),
            Arrays.asList("අකුණු ගහන්නේ","අකුනූ ගහන්නේ")
    );
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.song2_reading);
        ...
        databaseReference = FirebaseDatabase.getInstance().getReference().child("song2").child(currentUserId);
    ...
    private void loadCorrectnessState() {
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(DatabaseError databaseError) {
                Toast.makeText(Song2_readingActivity.this, "Failed to load data.", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void storeResultInFirebase(int correctCount) {
        ...
        resultReference.setValue(correctCount).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Toast.makeText(Song2_readingActivity.this, "Result saved successfully.", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(Song2_readingActivity.this, "Failed to save result.", Toast.LENGTH_SHORT).show();
            }
        });
    }
